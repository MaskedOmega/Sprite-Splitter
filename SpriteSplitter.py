...
class SpriteSheet:
    ...
    def get_folders(self,look_path):
        ...
        folders = [folder.name.lower() for folder in look_path.iterdir() if folder.is_dir()]
        return(folders)
    ...
    def run_multi(self,SpriteXY,save_path,look_path):
        ...
        for f in range(len(folders)):
            ...
            for i in range (0,self.maxSizeY,self.SpriteXY[1]):
                count = 0
                for j in range (0,self.maxSizeX,self.SpriteXY[0]):
                    self.save_sprite(j,i,SpriteXY[0],SpriteXY[1], path + "/" + str(folders[f]) +"_"+ str(pos) +"_"+ str(count) + ".png")
                    count += 1
                pos +=1
